...
SRC = Path("docs/report/To_nhiem_vu_va_chuong_1_2_Lutest_Lutest_chuong_2_co_anh.docx")
OUT = Path("docs/report/To_nhiem_vu_va_chuong_1_2_Lutest_Lutest_chuong_2_cap_nhat_ly_thuyet.docx")
...
replacement = "".join(
    [
        ...
        para(
            "Playwright là thư viện tự động hóa trình duyệt được sử dụng làm nền tảng cho Scan Engine của Lutest. "
        ...
        para(
            "Dựa trên dữ liệu hình học của DOM, Lutest kiểm tra các lỗi giao diện cơ bản như phần tử tràn khỏi viewport, nội dung bị cắt, layout bị lệch, phần tử bị che khuất hoặc hiển thị sai trên các kích thước màn hình desktop, tablet và mobile."
        ),
        para(),
        ...
        para(
            "Lutest sử dụng ý tưởng AABB để phát hiện các trường hợp chồng lấn giao diện như text bị đè, button bị che hoặc card layout chồng lên nhau. "
        ...
        para(
            "Đối với màu sắc, Lutest dựa trên nguyên tắc kiểm tra độ tương phản theo WCAG để đánh giá khả năng đọc giữa màu chữ và màu nền. "
        ...
        para(
            "Trong Lutest, Project Graph được xây dựng bằng Node.js và TypeScript ở phía Worker Backend. Dữ liệu graph được chuẩn hóa thành JSON và cung cấp cho dashboard thông qua Express.js API. "
        ...
        para(
            "Kết quả kiểm thử được lưu local trong thư mục .lutest/ gồm report JSON, graph snapshot, screenshot và log. Screenshot đóng vai trò bằng chứng trực quan cho lỗi, giúp người dùng xem lại vị trí lỗi trên giao diện thay vì chỉ đọc mô tả văn bản."
        ),
        para(),
        ...
        para(
            "Trong định hướng hoàn chỉnh, Lutest có thể tích hợp CI/CD để chạy kiểm thử giao diện sau mỗi pull request hoặc trước khi triển khai. Kết quả scan có thể được xuất ra JSON/HTML hoặc gửi lên cloud, giúp nhóm phát triển phát hiện lỗi giao diện sớm, theo dõi lịch sử chất lượng sản phẩm và đặt ngưỡng cảnh báo khi số lỗi vượt mức cho phép."
        ),
    ]
)
