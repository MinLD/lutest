...
const isAuthErrorCode = (value) => value === "AUTH_STATE_MISSING" || value === "AUTH_STATE_INVALID" || value === "AUTH_STATE_WRITE_FAILED" || value === "AUTH_SESSION_START_FAILED" || value === "AUTH_SESSION_TIMEOUT";
...
const validateRuntimeScanRequest = (value) => {
    ...
    const keys = rejectUnknownKeys(value, ["enabled", "baseUrl", "routes", "targets", "discoveryMode", "viewportPreset", "auth"]);
    if (!keys.ok)
        return keys;
    ...
    if (value.auth !== undefined) {
        if (!isRecord(value.auth))
            return { ok: false, code: "INVALID_REQUEST", message: "runtimeScan.auth must be an object" };
        const authKeys = rejectUnknownKeys(value.auth, ["useSavedState"]);
        if (!authKeys.ok)
            return authKeys;
        if (value.auth.useSavedState !== true)
            return { ok: false, code: "INVALID_REQUEST", message: "runtimeScan.auth.useSavedState must be true" };
    }
...
const validateAuthStartRequest = (value) => {
    if (!isRecord(value))
        return { ok: false, code: "INVALID_REQUEST", message: "AuthStartRequest must be an object" };
    const keys = rejectUnknownKeys(value, ["projectPath", "baseUrl", "timeoutMs", "successSelector", "successUrlIncludes"]);
    if (!keys.ok)
        return keys;
    if (!isOptionalNonEmptyString(value.projectPath))
        return { ok: false, code: "INVALID_REQUEST", message: "projectPath must be a non-empty string" };
    const baseUrl = validateLocalRuntimeBaseUrl(value.baseUrl);
    if (!baseUrl.ok)
        return baseUrl;
    const timeoutMs = value.timeoutMs;
    if (timeoutMs !== undefined && (typeof timeoutMs !== "number" || !Number.isInteger(timeoutMs) || timeoutMs < 1_000 || timeoutMs > 300_000))
        return { ok: false, code: "INVALID_REQUEST", message: "timeoutMs must be 1000..300000" };
    if (value.successSelector !== undefined && (!isNonEmptyString(value.successSelector) || value.successSelector.length > 300))
        return { ok: false, code: "INVALID_REQUEST", message: "successSelector is invalid" };
    if (value.successUrlIncludes !== undefined && (!isNonEmptyString(value.successUrlIncludes) || /^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(value.successUrlIncludes) || value.successUrlIncludes.startsWith("//") || value.successUrlIncludes.length > 300))
        return { ok: false, code: "INVALID_REQUEST", message: "successUrlIncludes is invalid" };
...
exports.validateAuthStartRequest = validateAuthStartRequest;
const validateAuthError = (value) => {
    if (!isRecord(value))
        return runtimeInvalid("auth error must be object");
    const keys = rejectUnknownKeys(value, ["code", "message"]);
    if (!keys.ok)
        return keys;
    if (!isAuthErrorCode(value.code) || !isNonEmptyString(value.message))
        return runtimeInvalid("auth error invalid");
    return { ok: true, value: { code: value.code, message: value.message } };
};
const validateAuthStateSummary = (value) => {
    if (!isRecord(value))
        return runtimeInvalid("auth state must be object");
    const keys = rejectUnknownKeys(value, ["exists", "valid", "savedAt", "updatedAt", "expiresAt", "storageStateRef"]);
    if (!keys.ok)
        return keys;
    if (typeof value.exists !== "boolean" || typeof value.valid !== "boolean")
        return runtimeInvalid("auth state flags invalid");
    if (!isOptionalString(value.savedAt) || !isOptionalString(value.updatedAt) || !isOptionalString(value.expiresAt))
        return runtimeInvalid("auth state dates invalid");
    if (value.storageStateRef !== undefined && !isSafeArtifactRef(value.storageStateRef))
        return runtimeInvalid("auth storageStateRef invalid");
...
const validateAuthStatusResponse = (value) => {
    if (!isRecord(value))
        return runtimeInvalid("AuthStatusResponse must be object");
    const keys = rejectUnknownKeys(value, ["status", "exists", "valid", "savedAt", "updatedAt", "expiresAt", "storageStateRef", "error"]);
    if (!keys.ok)
        return keys;
    if (value.status !== "missing" && value.status !== "valid" && value.status !== "invalid")
        return runtimeInvalid("auth status invalid");
    const summary = validateAuthStateSummary({ exists: value.exists, valid: value.valid, savedAt: value.savedAt, updatedAt: value.updatedAt, expiresAt: value.expiresAt, storageStateRef: value.storageStateRef });
    if (!summary.ok)
        return summary;
    const error = value.error === undefined ? undefined : validateAuthError(value.error);
    if (error && !error.ok)
        return error;
    return { ok: true, value: { status: value.status, ...summary.value, error: error?.value } };
};
exports.validateAuthStatusResponse = validateAuthStatusResponse;
const validateAuthStartResponse = (value) => {
    if (!isRecord(value))
        return runtimeInvalid("AuthStartResponse must be object");
    const keys = rejectUnknownKeys(value, ["status", "authState", "error"]);
    if (!keys.ok)
        return keys;
    if (value.status !== "saved" && value.status !== "timeout" && value.status !== "failed")
        return runtimeInvalid("auth start status invalid");
    const authState = value.authState === undefined ? undefined : validateAuthStateSummary(value.authState);
...
exports.validateAuthStartResponse = validateAuthStartResponse;
const validateAuthClearResponse = (value) => {
    if (!isRecord(value))
        return runtimeInvalid("AuthClearResponse must be object");
    const keys = rejectUnknownKeys(value, ["cleared", "status"]);
    if (!keys.ok)
        return keys;
    if (typeof value.cleared !== "boolean" || (value.status !== "cleared" && value.status !== "missing"))
        return runtimeInvalid("auth clear response invalid");
    return { ok: true, value: { cleared: value.cleared, status: value.status } };
};
exports.validateAuthClearResponse = validateAuthClearResponse;
