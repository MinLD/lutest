...
const endpoints = [
  ["Local", "http://localhost:4318/v1", "ready"],
  ["Express API", "http://localhost:8787/api", "live"],
  ["Worker node", "tcp://127.0.0.1:7011", "idle"],
];

const providers = [
  ["Next graph", "27 routes", "connected"],
  ["Express scan", "14 handlers", "connected"],
  ["Contracts", "6 schemas", "ready"],
  ["Reports", "3 drafts", "ready"],
];

const scans = [
  ["test_scan/next-graph-243", "passed", "1m 24s"],
  ["apps/worker-node", "passed", "42s"],
  ["packages/contracts", "warning", "18s"],
];
...
function NavGroup({ items }: { items: (string | boolean)[][] }) {
  return (
    ...
    <header className="mb-8 flex flex-col justify-between gap-5 sm:mb-11 lg:flex-row lg:items-start">
      <div>
        ...
        <p className="mt-3 max-w-2xl text-pretty text-base leading-7 text-[#5f6b7a] sm:text-lg">
          Clean view for local static scans. Inspired by proxy tooling, tuned
          for code graph work.
        </p>
      </div>

      <div className="flex items-center gap-2">
        <button className="items-center flex   rounded-xl border border-pink-200 bg-pink-50 px-3 py-2 text-sm font-bold text-pink-600 min-[430px] gap-2">
          <Rose /> Donate
        </button>
        <button
          type="button"
  ...
  return (
    <section className="rounded-[1.35rem] bg-white p-4 shadow-[0_1px_0_#dbe7f5,0_18px_50px_rgba(36,63,103,0.06)] sm:p-7">
      <SectionTitle
        icon="⌁"
        title="API endpoints"
        subtitle="Local services used by the dashboard"
      />

      <div className="mt-6 space-y-3 sm:mt-7">
        {endpoints.map(([label, value, state]) => (
          <div
            key={label}
  ...
  return (
    <section>
      <div className="mb-4 flex items-end justify-between gap-4">
        ...
        />
        <button className="hidden rounded-xl border border-[#dbe7f5] bg-white px-3.5 py-2 text-sm font-semibold text-[#526071] transition hover:border-[#2563eb]/50 sm:block">
          Test all
        </button>
      </div>
  ...
  return (
    <section className="rounded-[1.35rem] bg-white p-4 shadow-[0_1px_0_#dbe7f5,0_18px_50px_rgba(36,63,103,0.06)] sm:p-7">
      <SectionTitle
        icon="◇"
        title="Route graph"
        subtitle="Static preview from latest scan"
      />
      ...
      <div className="mt-6 min-h-[18rem] rounded-[1.1rem] border border-dashed border-[#dbe7f5] bg-[#fbfdff] p-4 sm:mt-7 sm:min-h-[22rem] sm:p-5">
        <div className="grid h-full gap-4 md:grid-cols-[1fr_1.1fr_0.9fr]">
          <GraphColumn
            title="Routes"
            rows={["app/page.tsx", "dashboard/page.tsx", "api/report/route.ts"]}
          />
          <GraphColumn
            title="Components"
            rows={[
          ...
          <GraphColumn
            title="Outputs"
            rows={["graph.json", "report.md", "warnings[]"]}
          />
        </div>
      </div>
    </section>
  ...
  return (
    <aside className="rounded-[1.35rem] bg-white p-4 shadow-[0_1px_0_#dbe7f5,0_18px_50px_rgba(36,63,103,0.06)] sm:p-6">
      <SectionTitle
        icon="▣"
        title="Latest runs"
        subtitle="Recent local checks"
      />

      <div className="mt-6 space-y-4">
        {scans.map(([target, state, time]) => (
          <div
            key={target}
          ...
          >
            ...
            <p
              className={
                state === "warning"
                  ? "mt-2 text-sm font-semibold text-[#c98216]"
                  : "mt-2 text-sm font-semibold text-[#0d9b51]"
              }
            >
              {state}
            </p>
